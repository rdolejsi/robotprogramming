...
if __name__ == "__main__":
    ...
    while not button_a.is_pressed():
        # Tries to maintain the robot at the sweet spot around 20 centimeters in front of an obstacle.
        # We use the Sonar sensor to measure the distance to the obstacle and gradually increase
        # the speed of the robot to 30 cm/s. We also support backing off from the obstacle when
        # too close. We allow for +/- 2 centimeters around the target distance to avoid oscillation.
        # Please note: issuance of stop on direction reversal is a built-in Wheel feature,
        # we don't need to concern ourselves with it.
        distance_cm = sonar.get_distance_cm()
        if distance_cm > 21:
            # When moving forward, we increase the PWM speed gradually using distance error correction
            # and a predefined speed increase multiplier. The speed is limited to 30 cm/s.
            ...
            # If the distance is less than 15 cm, the robot stops and moves backwards
            # until the distance is greater than 20 cm. The reverse direction is constant for now.
            wheel_driver.move_pwm_for_distance(backoff_speed_pwm, backoff_speed_pwm, 20 - distance_cm)
