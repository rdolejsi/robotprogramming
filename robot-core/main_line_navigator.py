...
STATES = dict(
    ...
    #  Left sharp turn
    TURN_L = State("Turn-L", '\\', 0b110, [ACTIONS["START_TURN_L"], ACTIONS["CONT_TURN_L"], ACTIONS["FINISH_TURN_L"]]),
    #  Right sharp turn
    TURN_R = State("Turn-R", '/', 0b011, [ACTIONS["START_TURN_R"], ACTIONS["CONT_TURN_R"], ACTIONS["FINISH_TURN_R"]]),
    #  T-crossing
    CROSS_T = State("Cross-T", 'T', 0b111, [ACTIONS["STOP"]]),
    #  intersection, Y shape
    CROSS_Y = State("Cross-Y", 'Y', 0b101, [ACTIONS["STOP"]]),
    #  stops the robot
...
if __name__ == "__main__":
    ...
    line_losing_rotation_rad_start = 3
    line_losing_cycle = 0
    line_losing_rotation_rad_increment_per_cycle = 10
    line_losing_cycle_multiplier = 3
    rotation_max = line_losing_rotation_rad_start * 7
    forward_speed = 10  # radians per second
    turn_speed = 4  # radians per second
    forward_align_speed_degrader_cycle_multiplier = 5
    forward_align_speed_min = 5
    system.display_on()
    ...
    try:
        ...
        while not button_a.is_pressed():
            ...
            if ticks_diff(time_now, regulation_cycle_start) > regulation_cycle_length:
                ...
                elif lcr == 0b111 or lcr == 0b110 or lcr == 0b011:
                    state = STATES["START"]
                    action_idx = 0
                    action = state.actions[action_idx]
                    system.display_drive_mode(state.symbol)
