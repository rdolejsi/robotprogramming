...
class StateMap:
    ...
    def __init__(self, stop_on_non_line_sensors=False, intersections=False):
        ...
        stop_matchers = None if not stop_on_non_line_sensors else [
            SensorHistoryStateMatcher(steps=[SensorHasCount(sensor=0b111, min_count=3)]),
            SensorHistoryStateMatcher(steps=[SensorHasCount(sensor=0b011, min_count=3)]),
            SensorHistoryStateMatcher(steps=[SensorHasCount(sensor=0b110, min_count=3)]),
        ]
        self.states = {
            # Generic states
            'START': StartState(symbol='s'),
