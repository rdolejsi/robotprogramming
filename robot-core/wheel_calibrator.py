...
class WheelCalibrator:
    def __init__(self, wheel: Wheel):
        ...
        self.wheel = wheel
        self.pwm_speed_to_rad_per_sec = [0.0 for _ in range(wheel.pwm_min, wheel.pwm_max + 1)]
    ...
    def gather_pwm_to_real_speed_table_fast_approx(self):
        ...
        while speed_pwm <= self.wheel.pwm_max:
            ...
            self.pwm_speed_to_rad_per_sec[speed_pwm - self.wheel.pwm_min] = rad_per_sec
            if rad_per_sec > 0.0:
        ...
        rad_speed_delta = (self.pwm_speed_to_rad_per_sec[self.pwm_speed_max - self.wheel.pwm_min]
                           - self.pwm_speed_to_rad_per_sec[0])
        a = rad_speed_delta / (self.pwm_speed_max - self.pwm_speed_min)
        b = self.pwm_speed_to_rad_per_sec[0] - a * self.pwm_speed_min
        for speed_pwm in range(self.wheel.pwm_min, self.wheel.pwm_max + 1):
            speed_write = a * speed_pwm + b
            self.pwm_speed_to_rad_per_sec[speed_pwm - self.wheel.pwm_min] = speed_write
        self.calculate_pwm_value_multiplier_and_shift()
        self.conversion_table_available = True
    ...
    def calculate_pwm_value_multiplier_and_shift(self):
        ...
        for pwm in range(self.wheel.pwm_min, self.wheel.pwm_max + 1):
            ...
            sum_rad += self.pwm_speed_to_rad_per_sec[pwm - self.wheel.pwm_min]
            sum_pwm_mult_rad += pwm * self.pwm_speed_to_rad_per_sec[pwm - self.wheel.pwm_min]
    ...
    def calibration_table_to_csv(self):
        ...
        for pwm in range(self.wheel.pwm_min, self.wheel.pwm_max + 1):
            ...
            if pwm < self.wheel.pwm_min + 5 or pwm > self.wheel.pwm_max - 5:
                rad = self.pwm_speed_to_rad_per_sec[pwm - self.wheel.pwm_min]
                cm = self.wheel.encoder.radians_to_cm(rad)
